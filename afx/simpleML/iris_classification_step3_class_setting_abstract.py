...
class ClassificationML(ABC):
    ...
    def run(self):
        ...
        # 3. 데이터 전처리
        col = self.set_drop_cols()
        self.df.drop(col,axis=1,inplace=True) # 불필요한 열(ID) 제거: axis=1 : 컬럼, inplace=1 : 삭제 후 데이터 프레임에 반영 

        # 4. 데이터 시각화

        # 모든 컬럼 쌍에 대해 plot 시각화 - 'Species'에 따라 색을 달리 시각화 
        h = self.set_hue()
        sns.pairplot(self.df, hue=h)
        plt.show()
        ...
        train, test = train_test_split(self.df, test_size = 0.3)# in this our main data is split into train and test
        in_cols = self.set_in_cols()
        answer_col = self.set_answer_col()

        train_X = train[in_cols]# taking the training data features
        train_y = train[answer_col] # output of our training data

        test_X = test[in_cols] # taking test data features
        test_y =test[answer_col]   #output value of test data
...
class IrisClassification (ClassificationML):
    def load_csv(self):
    ...
    def set_answer_col(self):
        return 'Species'


class MaleFemaleClassification(ClassificationML):
    def load_csv(self):
...
if __name__ == "__main__":
    gildong = MaleFemaleClassification()
    gildong.run()
